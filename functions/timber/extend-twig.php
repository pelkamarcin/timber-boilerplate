...
function themeprefix_array_merge_recursive( $arr1, $arr2 ) {
    if ( $arr1 instanceof Traversable ) {
        ...
        throw new Twig_Error_Runtime( sprintf( 'The merge_recursive filter only works with arrays or "Traversable", got "%s" as first argument.',
                                               gettype( $arr1 ) ) );
    }
    if ( $arr2 instanceof Traversable ) {
        $arr2 = iterator_to_array( $arr2 );
    } elseif ( !is_array( $arr2 ) ) {
        throw new Twig_Error_Runtime( sprintf( 'The merge_recursive filter only works with arrays or "Traversable", got "%s" as second argument.',
                                               gettype( $arr2 ) ) );
    }
...
function themeprefix_string_slugify( $str ) {
    if ( gettype( $str ) === 'string' ) {
        ...
        throw new Twig_Error_Runtime( sprintf( 'The slugify filter only works with strings, got "%s" as first argument.',
                                               gettype( $str ) ) );
    }
